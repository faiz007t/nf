...
function MediaUnlockTest_Viu() {
    echo -n -e " Viu:\t\t\t\t->\c";
    local result=`curl -sSL -${1} "https://www.viu.com/ott/" + "${country_code}" 2>&1`;
    if [[ "$result" == "curl"* ]];then
        return
    fi
...
function ISP(){
	...
	local code=$(PharseJSON "${result}" "country_code" 2>&1)
    local isp="$(PharseJSON "${result}" "isp" 2>&1) [$(PharseJSON "${result}" "country_code" 2>&1)]";
    if [ $? -eq 0 ];then
        echo " ** IP: ${ip}"
